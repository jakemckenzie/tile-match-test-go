...
func drawGem(screen *ebiten.Image, pts []Point, col color.RGBA, blockX, blockY, blockSize float32) {
    // TODO
}


func drawBlock(screen *ebiten.Image, b Block, gx, gy int, blockSize float32) {
    pts := getTransformedPoints(b.Shape)
    col := getShapeColor(b.Shape)
    blockX := float32(gx) * blockSize
    blockY := float32(gy) * blockSize
    drawGem(screen, pts, col, blockX, blockY, blockSize)
    if b.Selected {
        // TODO
    }
}
...
func (g *Game) Draw(screen *ebiten.Image) {
    ...
    // text.Draw(screen, "Score: "+strconv.Itoa(g.world.Score), ebitenutil.DebugFont, 10, 20, color.Black)
    // text.Draw(screen, "Combo: "+strconv.Itoa(g.world.Combo), ebitenutil.DebugFont, 10, 40, color.Black)
    // text.Draw(screen, "Instructions: Match at least 4 blocks", ebitenutil.DebugFont, 10, 60, color.Black)
}
